...
for url in "${BLOCKLISTS[@]}"; do
  ...
  elif [[ $HTTP_RC -eq 429 ]]; then
    echo -e "\\nRate limit exceeded (${HTTP_RC}): $url"
    sleep 60  # Espera de 60 segundos antes de continuar
  else
...
# Cria um arquivo de restauração para o ipset
cat >| "$IP_BLOCKLIST_RESTORE" <<EOF
create $IPSET_TMP_BLOCKLIST_NAME -exist hash:net family inet hashsize ${HASHSIZE:-32768} maxelem ${MAXELEM:-131072}
create $IPSET_BLOCKLIST_NAME -exist hash:net family inet hashsize ${HASHSIZE:-32768} maxelem ${MAXELEM:-131072}
EOF

# Adiciona as IPs ao ipset temporário
sed -rn -e '/^#|^$/d' \
